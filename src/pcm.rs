...
impl<'a> SwParams<'a> {
    ...
    pub fn set_tstamp_type(&self, v: TstampType) -> Result<()> {
        acheck!(snd_pcm_sw_params_set_tstamp_mode((self.1).0, self.0, v as c_uint)).map(|_| ())
    }

    pub fn get_tstamp_type(&self) -> Result<TstampType> {
        let mut v = 0;
        acheck!(snd_pcm_sw_params_get_tstamp_type(self.0, &mut v)).map(|_| v != 0)
            .and_then(|_| TstampType::from_c_int(v as c_int, "snd_pcm_sw_params_get_tstamp_type"))
    }
